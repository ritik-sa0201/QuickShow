        ...
        <table className="w-full border-collapse rounded-md overflow-hidden whitespace-nowrap">
          ...
          <tbody className="text-sm font-light">
            {bookings.map((item, index) => (
              <tr
                key={index}
                className="border-b border-[var(--color-primary)]/20 bg-[var(--color-primary)]/5 even:bg-[var(--color-primary)]/10"
              >
                <td className="p-2 min-w-45 pl-5">{item.user.name}</td>
                <td className="p-2">{item.show.movie.title}</td>
                <td className="p-2">{dateFormaat(item.show.showDateTime)}</td>
                <td className="p-2">
